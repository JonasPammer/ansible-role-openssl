...
Wheter to add the Certificate to the
https://serverfault.com/questions/62496/ssl-certificate-location-on-unix-linux[System's CA Trust Store]
by copying the certificate to specific places and using
`update-ca-certificates` (Debian) / `update-ca-trust` (RHEL)
to integrate them into the store.
